...
var pacterahelix = require("./scripts/pacterahelix.js");
...
gulp.task("05-Sync-Unicorn", function (callback) {
    var options = {};
    options.siteHostName = pacterahelix.getSiteUrl();
...
gulp.task("Publish-Assemblies", function () {
    var root = "./src";
    var binFiles = root + "/**/code/**/bin/Sitecore.{Feature,Foundation,Project}.*.{dll,pdb}";
...
gulp.task("Auto-Publish-Assemblies", function () {
    ...
    var files = "/**/Sitecore.{Feature,Foundation,Project}.*.{dll,pdb}";;
    var destination = config.websiteRoot + "/bin/";
    gulp.src(roots, { base: root }).pipe(
      foreach(function (stream, rootFolder) {
          gulp.watch(rootFolder.path + files, function (event) {
...
gulp.task("Package-Prepare-Package-Files", function (callback) {
    var excludeList = [
      ...
      "!" + config.websiteRoot + "\\bin\\Sitecore.{Feature,Foundation,Project,Demo,Common}*dll"
    ];
